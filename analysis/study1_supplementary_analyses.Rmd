...
(plot_h1 = predicted %>%
  ...
  geom_point(data = ind_data, aes(x, predicted, color = roi), alpha = .1, size = .25, position = position_jitter(height = .1)) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = roi), alpha = .2, color = NA) +
...
predicted_h6 %>%
  ggplot(aes(x = x, y = predicted)) +
  geom_point(data = ind_data_h6, aes(x, predicted, color = cond), position = position_jitter(.1, .1), alpha = .1, size = .5) + 
...
`r emo::ji("check")` H7a: The effect of focus-on-self intervention on sharing intention will be mediated by increased activity in the self-referential ROI.
...
`r emo::ji("x")` H7b: The effect of focus-on-other intervention on sharing intention will be mediated by increased activity in the mentalizing ROI.
