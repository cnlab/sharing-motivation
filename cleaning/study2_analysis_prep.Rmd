...
```{r}
data_raw = read.csv("../data/raw/study2_data.csv", stringsAsFactors = FALSE)
...
data = messages %>%
  ...
  select(group, SID, item, article_cond, trial, topic, sharing_type_key, sharing_type, msg_share, contains("rel_"), comment, n_words)  %>%
...
```{r}
demo = read.csv("../data/raw/study2_demo.csv", stringsAsFactors = FALSE) %>%
...
# write csvs
```{r}
write.csv(data, "../data/study2_data.csv", row.names = FALSE)
